...
int main() {
    ...
    mb_mapping = modbus_mapping_new(
        10,  // 1 Discrete input 	(Digital Output skriver til)
        10,  // 1 Discrete output 	(Digital Input læser fra)
        10, 10  // Ingen holding/input registers
    );
    if (!mb_mapping) {
    ...
    while (true) {
	    ...
	    //Hvis ingen forespørgsler modtages, vent 100 ms for at undgå unødigt CPU-forbrug
			std::cout <<  "disconnecy, errno? " <<  errno <<  " : " <<  modbus_strerror(errno) <<  std::endl;	
            modbus_tcp_accept(ctx, &server_socket);
        }
    }
