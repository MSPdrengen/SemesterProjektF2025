...
/*
Kompiler:
	g++ -std=c++17 -o finishGripper finishGripper.cpp -lmodbus -lsqlite3 -lpthread -lpigpio
Kør: 
...
*/    
    ...
    while (true) {
        ...
        if (mb_mapping->tab_bits[0] == 0 && val26) {
            ...
            std::cout << "Lukker gripper val24: " << val24  <<  " val 26: "  << val26  <<  std::endl;
            gpioPWM(GPIO_7, 0);
            gpioPWM(GPIO_8, 50);
        }
        ...
        if (val26 != previousVal26) {
			previousVal26 = val26;
			std::chrono::high_resolution_clock::time_point stop = std::chrono::high_resolution_clock::now();
            std::chrono::duration<long long> duration = std::chrono::duration_cast <std::chrono::seconds>(stop - start);
            std::cout << "Duration: " << duration.count();
		} else if(val24 != previousVal24){
			previousVal24 = val24;
			std::chrono::high_resolution_clock::time_point stop = std::chrono::high_resolution_clock::now();
            std::chrono::duration<long long> duration = std::chrono::duration_cast <std::chrono::seconds>(stop - start);
            std::cout << "Duration: " << duration.count();
